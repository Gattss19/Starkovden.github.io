...
{% include image.html file="introduction-rest-api/27.png" alt="calc" caption="Когда нажимаем кнопки, скрытые функции взаимодействуют с другими компонентами и выдают информацию" %}
...
{% include image.html file="introduction-rest-api/28.jpg" alt="diagram" %}
...
{% include tip.html content="Отношения между ресурсами и методами часто описывается в терминах «существительные» и «глаголы». Ресурс - это существительное, потому что это объект или вещь. Глагол - это то, что вы делаете с этим существительным. Объединяя существительные с глаголами, вы формируете язык REST." %}

Более подробно рассмотрим конечные точки в следующих разделах (например, в [Обзоре руководства описания API](api-reference-tutorial-overview.html) мы рассмотрим каждое свойство ресурса). А здесь дан краткий обзор.
...
{% include image.html file="introduction-rest-api/29.jpg" alt="REST" %}

 Можно увидеть этот ответ в curl, если открыть окно терминала и набрать `curl` [https://idratherbewriting.com](https://idratherbewriting.com). (Предполагается, что [curl установлен ](curl-intro-and-instalation.html))
...
Некоторые формальные спецификации - например, [OpenAPI](introduction-openapi-and-swagger.html) и [RAML](RAML-tutorial.html) - были разработаны для описания API REST. Когда вы описываете свой API с использованием спецификации OpenAPI или RAML, инструменты, которые могут читать эти спецификации (например, [Swagger UI](swagger-ui-tutorial.html) или [консоль RAML API](RAML-tutorial.html#consoleOutputs)), будут генерировать вывод интерактивной документации.

Спецификация OpenAPI может заменить файл WSDL, более распространенный в SOAP. Такие инструменты, как [Swagger UI](swagger-ui-tutorial.html), которые читают документы спецификаций, обычно создают интерактивную документацию (с использованием API-консолей или API-проводников) и позволяют вам тестировать вызовы REST и просматривать ответы прямо в браузере.

Но не ожидайте, что выходные данные документации Swagger UI или RAML API Console будут содержать все детали, которые потребуются пользователям для работы с вашим API. Например, эти выходные данные не будут включать информацию о [ключах авторизации](authentication-and-authorization.html), подробностях о рабочих процессах и взаимозависимостях между конечными точками и так далее. Вывод Swagger или RAML обычно содержит только адресную документацию, на которую обычно приходится только треть или половина всей необходимой документации (в зависимости от API).
